...
Current version: `0.2.0` - [Changelog](/docs/changelog)
...
Version 0.2.0 introduces **Solana-native privacy analysis**:

### New Solana-Specific Data
The `ScanContext` now includes:
- `feePayers` - Who pays transaction fees (critical linkage vector)
- `signers` - All signers across transactions (authority patterns)
- `transactions` - Full transaction metadata (fee payers, signers, memos)
- `tokenAccountEvents` - Token account lifecycle (create/close events)
- `pdaInteractions` - Program-Derived Address usage patterns
- `programs` - All programs interacted with

### New Heuristics
Four new Solana-specific heuristics:
1. **Fee Payer Reuse** (CRITICAL) - Detects managed accounts and bot infrastructure
2. **Signer Overlap** (HIGH) - Tracks multi-sig patterns and authorities
3. **Instruction Fingerprinting** (MEDIUM) - Behavioral signatures
4. **Token Account Lifecycle** (MEDIUM) - Rent refund trail analysis

### Enhanced Existing Heuristics
- **Counterparty Reuse** - Now PDA and program-aware
- **Amount Reuse** - Downgraded severity (common on Solana)

### Backwards Compatible
All existing code continues to work. New data is additive.
...
- **[CLI Usage](/docs/cli/quickstart)** - Use the command-line tool
- **[Understanding Reports](/docs/reports/risk-levels)** - Interpret scan results
- **[Contributing](/docs/contributing/development)** - Extend the library
